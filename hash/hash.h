...
class CHash
{
  ...
  public:
    ...
    требуется. Все создаваемые листики списков разрешения коллизий храним в менеджере памяти.
    */
    CHash(int hashTableSize, int defaultBlockSize)
        : m_tableSize(hashTableSize), m_pTable(new leaf *[hashTableSize]), m_Memory(defaultBlockSize, true)
    {
    ...
    */
    ...
    {
        unsigned int idx = 0;
        leaf *elem_leaf = findLeaf(pElement, idx);
    ...
    {
        unsigned int idx = 0;
        leaf *elem_leaf = findLeaf(&element, idx);
  ...
  private:
    ...
    */
    leaf *findLeaf(const T *pElement, unsigned int &idx)
    {
        ...
        while (curr != nullptr)
        {
            if (Compare(curr->pData, pElement) == 0)
            {
                break;
            }
            curr = curr->pnext;
    ...
    void pushLeaf(T *pElement, const unsigned int &idx)
    {
        try
        {
