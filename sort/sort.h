...
void merge(void **ppArray, void **ppSorted, int left, int mid, int right, CompareSortType pCompareFunc);
...
void simpleSort(void **ppArray, int length, CompareSortType pCompareFunc);
...
template <class T> void simpleSort(T **ppArray, int length, int(pCompareFunc)(const T *pElem1, const T *pElem2))
{
    simpleSort(reinterpret_cast<void **>(ppArray), length, (CompareSortType *)pCompareFunc);
}
}; // namespace templates
